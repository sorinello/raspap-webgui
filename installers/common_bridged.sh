...
function download_latest_files() {
    ...
    git clone --depth 1 https://github.com/sorinello/raspap-webgui /tmp/raspap-webgui || install_error "Unable to download files from github"
...
function default_configuration_bridged() {
    ...
    if [ "$answer" != 'n' ] && [ "$answer" != 'N' ]; then
        install_log "Enabling RaspAP_bridged daemon"
        echo "Disable with: sudo systemctl disable raspap_bridged.service"
        sudo cp $webroot_dir/installers/raspap_bridged.service /lib/systemd/system/ || install_error "Unable to move raspap_bridged.service file"
        sudo systemctl enable raspap_bridged.service || install_error "Failed to enable raspap_bridged.service"
    fi
}
...
function install_raspap_bridge() {
    display_welcome
    config_installation
...
sudo apt-get install -y git hostapd dnsmasq vnstat bridge-utils && install_log "Installing required packages"
